...
4. **测试与证据**：870 个 pytest 用例 + 覆盖率门禁 97% + CI（GitHub Actions）+
   Allure 报告——"测了、测够、能证明"；
...
│     ▼                                                                 │
│  验证层      pytest 套件（802 用例）+ hypothesis 属性测试              │
...
本项目 `tcms/tcms.dbc` 定义 8 个核心报文（模拟设计，非真实车型协议）：
...
98.00% 是 `pytest-cov` 实测值，门禁在 `pyproject.toml`（`fail_under=97`，
CI 与本地共用单源）。未覆盖的部分是
...
`docs/safety_case.md` 用 EN 50128 思路做安全论证映射：安全需求 SR-01~52 →
实现模块 → 测试证据 → 覆盖率证据。
...
**Q: 覆盖率为什么不是 100%？**
A: 98.00% 实测值 + 门禁。剩余为刻意保留的防御分支，QA 文档逐行说明。
