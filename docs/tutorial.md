...
4. **测试与证据**：652 个 pytest 用例 + 覆盖率门禁 97% + CI（GitHub Actions）+
   Allure 报告——"测了、测够、能证明"；
...
│     ▼                                                                 │
│  验证层      pytest 套件（658 用例）+ hypothesis 属性测试              │
...
97.98% 是 `pytest-cov` 实测值，CI `--cov-fail-under=97` 门禁。未覆盖的部分是
...
**Q: 覆盖率为什么不是 100%？**
A: 97.98% 实测值 + 门禁。剩余为刻意保留的防御分支，QA 文档逐行说明。
