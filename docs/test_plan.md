...
| 测试环境 | 纯 Python 3.10+ 仿真（CI ubuntu × 3.10/3.11/3.12）；本地 venv |
...
| 回归 Regression | （默认全量） | 全部 650+ 用例 | main 合并 / 发版前 | ~1 min |
...
- 全量 `pytest tests/ --cov=tcms` **707+ 用例全绿**（硬件用例显式跳过）
