...
| 证据基线 | 958 用例（pytest collect，2026-09-08 实测）· 98.00% 语句覆盖率（2651 stmts / 55 miss）· CI 全绿 |
...
> 用例数为 `pytest --collect-only` 实测（2026-09-08 基线，958 collected =
> 957 passed + 1 hardware skip，44 个测试文件）。
...
| 全绿基线 | 958 用例 · CI run 全绿（pr-smoke + lint + test 3.10/3.11/3.12/3.13 + demo-smoke + dist-smoke） |
