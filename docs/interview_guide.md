...
> 所有数字均为实测口径：**777 个 pytest 用例全绿、覆盖率 98.00%（2600 语句/52 未覆盖）、
...
**结果（Result）**：777 个仿真验证场景 100% 按预期通过，关键报文（心跳、超速报警、
...
> 刹车动作有三重确认。777 个验证场景全部通过，一条命令就能演示全过程，还能回放
...
网络与时间 多网段拓扑、虚拟时间基、故障生命周期、回放链
   ↓
验证层   pytest 777 用例 + 覆盖率门禁 97% + CI + Allure 报告
```
...
#   777 collected = 776 passed + 1 hardware skip
...
| 口径 | 数值 |
|------|------|
| 自动化用例 | 777 collected = 776 passed + 1 hardware skip |
