...
> 所有数字均为实测口径：**902 个 pytest 用例（901 passed + 1 hardware skip）、覆盖率 98.00%（2611 语句/53 未覆盖）、
...
**结果（Result）**：902 个自动化测试用例按预期通过（901 passed + 1 hardware skip），
...
> 刹车动作有三重确认。902 个验证场景按预期通过（901 passed + 1 hardware skip），一条命令就能演示全过程，还能回放
...
网络与时间 多网段拓扑、虚拟时间基、故障生命周期、回放链
   ↓
验证层   pytest 902 用例 + 覆盖率门禁 97% + CI + Allure 报告
```
...
#   902 collected = 901 passed + 1 hardware skip
...
| 口径 | 数值 |
|------|------|
| 自动化用例 | 902 collected = 901 passed + 1 hardware skip |
...
| 故障字典 | 97 条 F-TCMS（faults.yaml，FMEA 字段，13 系统域） |
| 场景库 | 75 个 YAML 事件式故障场景（97 FMEA 键全覆盖） |
| 安全需求 | SR-01~52（safety_case.md + rtm.csv 追溯） |
| 测试分层 | smoke 70 / safety 70 / 全量 902 |
