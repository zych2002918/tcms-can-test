...
> 所有数字均为实测口径：**930 个 pytest 用例（929 passed + 1 hardware skip）、覆盖率 98.00%（2611 语句/53 未覆盖）、
...
**结果（Result）**：930 个自动化测试用例按预期通过（929 passed + 1 hardware skip），
...
> 刹车动作有三重确认。930 个验证场景按预期通过（929 passed + 1 hardware skip），一条命令就能演示全过程，还能回放
...
网络与时间 多网段拓扑、虚拟时间基、故障生命周期、回放链
   ↓
验证层   pytest 930 用例 + 覆盖率门禁 97% + CI + Allure 报告
```
...
4. **覆盖率证据**：按 SIL 等级要求不同的覆盖率指标——SIL4 通常要求 MC/DC 级覆盖
   （本项目诚实说明：语句覆盖 98.00%，MC/DC 是真实认证要求，演示级未做）。
...
覆盖率 98.00% 实测，CI 门禁 97%；未覆盖的是刻意保留的防御分支，**诚实交代比硬凑 100%
更有说服力**。
...
#   930 collected = 929 passed + 1 hardware skip
...
| 口径 | 数值 |
|------|------|
| 自动化用例 | 930 collected = 929 passed + 1 hardware skip |
...
| 故障字典 | 135 条 F-TCMS（faults.yaml，FMEA 字段，13 系统域） |
| 场景库 | 89 个 YAML 事件式故障场景（135 FMEA 键全覆盖） |
| 安全需求 | SR-01~52（safety_case.md + rtm.csv 追溯） |
| 测试分层 | smoke 70 / safety 70 / 全量 930 |
