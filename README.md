...
**为什么不用 CANoe / 现成工具**：CANoe 是成熟商业工具，但它不展示"我理解协议层
在发生什么"。本项目把 CANoe 的核心能力（统计分析、Trace、错误统计、负载率）
逐一对标实现，每一步都有可讲解的物理/协议依据——面试要证明的不是"会用工具"，
而是"知道工具背后在算什么"。

**为什么不用 Robot Framework 等测试框架**：TCMS 报文测试的价值在协议级断言
（位级值域、周期抖动、ID 仲裁、错误计数），通用关键字框架需要大量自定义库才能
表达这些概念，反而掩盖测试意图；pytest + hypothesis 直接表达不变量。

**V-model 分层映射**：DBC（需求/协议定义）→ interlocks/ebm/ebr（安全逻辑设计）→
...
（无标定依据）；不做 CAN FD/位级物理层仿真（偏离测试框架定位，自研成本失控）；
不声称 SIL"认证"（模拟实现不等于认证，造假是红线）。
...
一键跑通全场景：**多节点仿真启动 → 超速报警 → BMS 节点失活 → 心跳看门狗判离线 →
...
GitHub Actions（`.github/workflows/ci.yml`）三个 job：

1. **test**：Python 3.10/3.11/3.12 矩阵，全部测试 + 覆盖率门禁（`--cov-fail-under=97`）
   + HTML 报告 artifact；
2. **lint**：ruff 全量静态检查（tcms/tests/demo/run/scripts）；
3. **demo-smoke**：冒烟运行 `demo.py` 与 `scripts/plot_timeline.py`（证明演示
   路径与绘图脚本在 CI 环境可用），上传时序图 artifact。
