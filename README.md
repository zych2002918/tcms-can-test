...
<!-- 自证：tests=902 (skipped 1, failures 0, errors 0) coverage=98.00% — 由 scripts/gen_badges.py 依据 JUnit + coverage.json 生成 -->
<!-- badges:end -->
...
- **902 个自动化用例**（44 文件）· 覆盖率门禁 **98.00%** · 属性测试（hypothesis）· 失败现场自动导出
- **覆盖测试工程师完整工作流**：故障字典（FMEA，97 条）→ 场景编排（YAML，75 个）→ 分层执行（冒烟/全量）
...
│  (DUT nodes)     │          │  (python-can)   │            │  902 cases    │
└─────────────────┘          └────────────────┘            └───────────────┘
...
| 全量回归 | （默认） | 全部 902 用例 + 覆盖率门禁 97% | 902 用例 | ~60s |
...
| 故障/场景 | `faultdb.py` + `faults.yaml`（FMEA 97 条）· `faultlevel.py` · `faultlife.py`（台账 + DSL）· `scenarios.py`（YAML 外部化） |
...
902 个用例逐文件设计详解见 [docs/test_cases.md](docs/test_cases.md)。
...
├── tests/                 # 902 用例（44 文件）+ conftest（共享总线/失败现场）
├── scenarios/*.yaml       # 75 个声明式故障场景（YAML 编排，97 个 FMEA 键全覆盖）
...
| [docs/test_cases.md](docs/test_cases.md) | 902 用例逐文件设计详解（含面试素材） | 测试/面试 |
...
- [x] **场景库按 FMEA 字典扩充**：8 → 75 个场景，97 条 FMEA 故障键全部可被场景消费（注入/恢复/断言三件套覆盖字典全键）
