...
def _speed_frame(ts: float, speed_kmh: float, speed_valid: bool = True) -> str:
    ...
    return (f"    {ts:.3f}  1  200   Rx   d 3 "
            f"{raw & 0xFF:02X} {(raw >> 8) & 0xFF:02X} {valid:02X}")


def _door_frame(ts: float, door_states=(0, 0, 0, 0)) -> str:
    b0 = (door_states[0] & 0x03) | ((door_states[1] & 0x03) << 2) \
        | ((door_states[2] & 0x03) << 4) | ((door_states[3] & 0x03) << 6)
    all_closed = 1 if all(s == 0 for s in door_states) else 0
    return (f"    {ts:.3f}  1  400   Rx   d 2 {b0:02X} "
            f"{(all_closed | 0x02):02X}")
...
def test_replay_door_open_while_moving_triggers_ebm():
    ...
    report = replay.ReplayChain.from_text(text).run()
    assert report["ebm_triggered"] is True
    details = {a["detail"] for a in report["alerts"]
               if a["kind"] == replay.ALERT_OVERRIDE_OVERRUN}
...
def test_replay_records_events_in_recorder():
    ...
    assert any(e["message"] and "ebm_trigger" in e["message"]
               for e in events)
...
def test_replay_report_after_run():
    ...
    text = "base hex timestamps absolute\n" + _speed_frame(0.1, 60.0)
    report = replay.ReplayChain.from_text(text).run()
    for key in ("frames", "alerts", "alert_kinds", "ebm_state",
                "ebm_triggered", "watchdog_states", "atp_last_level"):
...
def test_engine_with_custom_objects():
    ...
    engine = replay.ReplayEngine(frames, ebm_manager=mgr,
                                 atp_supervisor=sup, watchdog_table=wd)
    engine.run()
