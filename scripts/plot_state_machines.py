...
def _arrow(ax, x1, y1, x2, y2, label, color="#555555", ls="-",
           label_dx=0.0, label_dy=0.0, fs=8.5, rad=0.0, lw=1.4,
           label_ha="center"):
    ...
    if label:
        ...
        _label(ax, mx + label_dx, my + label_dy, label, color, fs=fs,
               ha=label_ha)


def _note(ax, x, y, text, color="#d62728", fs=9.5):
    """底部说明框（浅底 + 深边框，与主流程分层）。"""
    ax.text(x, y, text, ha="center", va="top", fontsize=fs, color="#8a1414",
            bbox=dict(boxstyle="round,pad=0.45", facecolor="#fff5f5",
...
def plot_ebm(cjk: bool):
    ...
    _arrow(ax, 2.18, 1.0, 2.82, 1.0,
           ...
           label_dy=0.12)
    # 自愈（绿色弧线，走下方避开主链）
    _arrow(ax, 0.85, 0.94, 1.05, 0.12, "self_heal(限1次)",
           color="#2e9e44", rad=-0.35, label_dx=-0.28, label_dy=0.25)
    # 自愈超限 → FAULT（橙色，从 BRAKE 顶部绕上）
    _arrow(ax, 1.3, 1.06, 2.7, 1.06, "自愈超限 → FAULT",
           color="#ff7f0e", rad=-0.15, label_dx=0.0, label_dy=0.1)
    # 司机缓解序列：BRAKE → WAIT_HANDLE_ZERO
    _arrow(ax, 0.9, 0.94, 0.9, 0.12, "prepare_release\n手柄回零",
           color="#8b5cf6", rad=0.25, label_dx=-0.38, label_dy=0.1)
    # WAIT_HANDLE_ZERO → WAIT_RELEASE_BTN
    _arrow(ax, 1.15, 0.0, 1.85, 0.0, "hold_release_button\n(<3s 可重试)",
           color="#8b5cf6", label_dy=-0.12)
    # WAIT_RELEASE_BTN → IDLE（保持≥3s 缓解成功）
    _arrow(ax, 2.1, 0.06, 0.12, 0.94, "保持≥3s\n缓解成功",
           color="#2e9e44", rad=-0.4, label_dx=0.1, label_dy=0.28)
    # FAULT 复位回 IDLE（虚线，底部走）
    _arrow(ax, 2.9, 0.94, 0.12, 0.9, "复位后\n重新可用", color="#555555",
           ls="--", rad=0.5, label_dx=0.0, label_dy=-0.15, lw=1.1)
...
def plot_atp(cjk: bool):
    ...
    _arrow(ax, 0.15, 1.0, 0.82, 1.0, f"v > 限速-{atp.WARNING_OFFSET_KMH:.0f}",
           color="#f5a623", label_dy=0.12)
    _arrow(ax, 1.18, 1.0, 1.82, 1.0, f"v > 限速-{atp.SBI_OFFSET_KMH:.0f}",
           color="#ff7f0e", label_dy=0.12)
    _arrow(ax, 2.18, 1.0, 2.82, 1.0, f"v > 限速-{atp.EBI_OFFSET_KMH:.0f}",
           color="#d62728", label_dy=0.12)
    # 降速回落：底部弧线，与正向箭头分层，错开不重叠
    _arrow(ax, 2.7, 0.92, 1.3, 0.92, "降速回落", color="#2e9e44",
           rad=-0.15, label_dy=-0.08, lw=1.1)
    _arrow(ax, 0.85, 0.92, 0.15, 0.92, "", color="#2e9e44",
           rad=0.15, lw=1.1)
...
def plot_errstate(cjk: bool):
    ...
    _arrow(ax, 1.68, 1.0, 2.82, 1.0, "TEC ≥ 256\n(发送错误连续)",
           color="#d62728", label_dy=0.12)
    # 恢复路径：底部弧线
    _arrow(ax, 2.9, 0.92, 0.1, 0.92, "128 位总线空闲\n自动恢复 error-active",
           color="#2e9e44", rad=-0.2, label_dy=-0.1, lw=1.1)
    _arrow(ax, 1.38, 0.92, 0.12, 0.92, "错误计数回落", color="#2e9e44",
           rad=-0.1, label_dy=-0.08, lw=1.1)

    _note(ax, 1.5, 0.35,
          "bus-off 期间节点离线：不参与收发（隔离故障节点，保护总线）")
    ax.set_xlim(-0.35, 3.35)
    ax.set_ylim(0.0, 1.5)
    ax.axis("off")
    if cjk:
...
def plot_door(cjk: bool):
    ...
    _arrow(ax, 0.15, 1.0, 1.32, 1.0, "开门指令\n(DoorOpenPermit)",
           color="#2e9e44", label_dy=0.12)
    _arrow(ax, 1.68, 1.0, 2.82, 1.0, "关门指令\n(AllDoorsClosed)",
           color="#2e9e44", label_dy=0.12)
    # Open → Fault（红色，上弧）
    _arrow(ax, 1.9, 1.06, 2.7, 1.06, "故障(卡滞/回路异常)",
           color="#d62728", rad=-0.2, label_dx=0.0, label_dy=0.1)
    # Fault → Closed（底部回退）
    _arrow(ax, 2.9, 0.94, 0.12, 0.92, "维修后复位", color="#2e9e44",
           rad=0.45, label_dx=0.0, label_dy=-0.18, lw=1.1)
    # Closed → Fault（左侧弧）
    _arrow(ax, 0.1, 0.94, 2.9, 0.1, "门状态异常 → Fault", color="#d62728",
           rad=0.2, label_dx=0.0, label_dy=0.2)
    # Open → Unknown（灰色，右下）
    _arrow(ax, 1.7, 0.94, 2.9, 0.1, "反馈丢失 → Unknown", color="#666666",
           rad=-0.3, label_dx=0.0, label_dy=0.2)
...
def plot_overspeed(cjk: bool):
    ...
    # 降速回落：两级底部弧线，错开
    _arrow(ax, 2.7, 0.92, 1.3, 0.92, "降速回落", color="#2e9e44",
           rad=-0.12, label_dy=-0.08, lw=1.1)
    _arrow(ax, 0.85, 0.92, 0.15, 0.92, "", color="#2e9e44",
           rad=0.12, lw=1.1)
    _label(ax, 0.5, 0.8, "降速回落", "#2e9e44", fs=8.5)
    # EBM → safe（走底部大弧，与顶部正向箭头分离；右→左弧线 rad 负值弯向下）
    _arrow(ax, 2.9, 0.85, 0.2, 0.85, "", color="#2e9e44",
           rad=-0.75, lw=1.1)
